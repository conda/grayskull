...
def mock_config():
    ...
    return Configuration(name="https://github.com/testuser/testpkg", version="1.0.0")
...
class TestGitHubRPackages:
    ...
    def test_get_cran_metadata_github_detection(
        self, mock_cran_index, mock_github_metadata, mock_is_github, mock_config
    ):
    ...
    def test_get_cran_metadata_cran_path(
        ...
        with (
            patch("grayskull.strategy.cran.download_cran_pkg") as mock_download,
    ...
    def test_version_tag_handling(self):
        ...
        config = Configuration(
            name="https://github.com/testuser/testpkg", version="1.0.0"
        )

        with (
            patch("grayskull.strategy.cran.handle_gh_version") as mock_handle,
            patch(
                "grayskull.strategy.cran.generate_git_archive_tarball_url"
            ) as mock_gen_url,
            patch("grayskull.strategy.cran.download_github_r_pkg") as mock_download,
            patch(
                "grayskull.strategy.cran.get_github_archive_metadata"
            ) as mock_metadata,
            patch("grayskull.strategy.cran.sha256_checksum") as mock_sha256,
        ):
            ...
            mock_sha256.return_value = "abcd1234567890"
            mock_metadata.return_value = {"Package": "testpkg", "orig_lines": []}
    ...
    def test_needs_compilation_handling(self):
        ...
        with (
            ...
            mock_gen_url.return_value = (
                "https://github.com/testuser/testpkg/archive/1.0.0.tar.gz"
            )
            mock_download.return_value = "/tmp/test.tar.gz"
            mock_sha256.return_value = "abcd1234567890"
            mock_metadata.return_value = {
